...
import com.screendead.noise.Input;
...
public class Window {
    ...
    public Window(String title, int width, int height, boolean isFullscreen, boolean vsyncEnabled) {
        ...
        monitor = Objects.requireNonNull(glfwGetMonitors()).get(0);
        ...
        glfwWindowHint(GLFW_SAMPLES, 0); // Enable MSAA
...
//            glfwSetInputMode(this.handle, GLFW_RAW_MOUSE_MOTION, GLFW_TRUE);
        ...
        camera = new Camera(new Vector3f(0.0f, 0.0f, -256.0f));
...
//            height = size.y;
    ...
    public void update() {
        ...
        renderer.pxl = Math.max(renderer.pxl - input.scrollAmount, 2.0f);
...
//        renderer.lampPos = camera.pos;
        ...
        renderer.setTransform(0, 0, 0,
                    0, 0, 0,
                    1.0f, 1.0f, 1.0f);
        renderer.ticks = (float) (System.currentTimeMillis() - start);
    }
    ...
    private void setIcon() {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            String path = Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("images/heart.png")).toURI().getPath().substring(1);

            System.out.println(path);

            ByteBuffer img = STBImage.stbi_load(path, w, h, channels, 4);
